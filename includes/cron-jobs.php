...
// Hinzufügen der Cron-Jobs bei Plugin-Aktivierung
...
// Entfernen der Cron-Jobs bei Plugin-Deaktivierung
...
function wpls_execute_follow_up_check() {
    ...
    $max_follow_ups = (int) ( $options['max_follow_ups'] ?? 3 );
    $days_between   = (int) ( $options['days_between_follow_ups'] ?? 3 );
    
    if ( $max_follow_ups <= 0 || $days_between <= 0 ) {
        return; // Automatisierung ist in den Einstellungen deaktiviert
    }
    
    $days_in_seconds = $days_between * DAY_IN_SECONDS;
    $current_time    = time();
    ...
    if ( $leads_query->have_posts() ) {
        while ( $leads_query->have_posts() ) {
            ...
            if ( $sent_count < $max_follow_ups ) {
                ...
                if ( $last_date <= ( $current_time - $days_in_seconds ) ) {
